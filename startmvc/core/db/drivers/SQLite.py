...
class SQLite(DbInterface):
    ...
    def __init__(self, config):
        ...
        # 确保数据库目录存在
        db_file = config.get('database', ':memory:')
        if db_file != ':memory:':
    ...
    def connect(self):
        """连接数据库"""
        try:
            db_file = self.config.get('database', ':memory:')
            ...
            raise DatabaseException(f"SQLite连接错误: {str(e)}")
    ...
    def query(self, sql, params=None):
        ...
        try:
            ...
            raise DatabaseException(f"SQLite查询错误: {str(e)}\nSQL: {sql}")
    ...
    def execute(self, sql, params=None):
        ...
        try:
            ...
            self.conn.rollback()
            raise DatabaseException(f"SQLite执行错误: {str(e)}\nSQL: {sql}")
